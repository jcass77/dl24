## bw150.py — BW150/DL24P Reader (Native Bluetooth)

Standalone script for reading the Atorch BW150 (DL24P V2) electronic load over Bluetooth SPP. Connects directly via Python's native RFCOMM socket — no `rfcomm` tool, no separate terminal, no `sudo`, no pairing required.

### Requirements

- Python 3.3+ (uses stdlib `socket.AF_BLUETOOTH`)
- `bluez` and `bluez-utils` installed
- `bluetooth.service` running
- BW150 powered on and in Bluetooth range
...
```bash
cd ~/projects/dl24
.venv/bin/python3 bw150.py              # single reading
.venv/bin/python3 bw150.py --listen 5   # stream 5 readings
.venv/bin/python3 bw150.py --listen 0   # continuous until Ctrl-C
.venv/bin/python3 bw150.py --json       # JSON output
.venv/bin/python3 bw150.py --addr XX:XX:XX:XX:XX:XX  # custom MAC
```

### Example Output

```
V:   12.6V  I:  0.800A  P:   10.1W  E:    1.23Wh  T: 28°C  t: 0h 15m 22s
```
...
| Problem | Solution |
|---|---|
| `Connection refused` on first attempt | Ensure BW150 is powered on; retry after 2-3 seconds |
| `No route to host` | Run `bluetoothctl power on` to enable the adapter |
| Timeout waiting for data | BW150 may need power cycle; check it's not connected to another device |
| `Permission denied` on socket | Add user to `lp` group: `sudo usermod -aG lp $USER` then re-login |
| `bluetoothctl pair` fails with `br-connection-refused` | **Expected** — this device does not support pairing. Use bw150.py directly |

### How It Works

1. Opens a native RFCOMM socket (`AF_BLUETOOTH` / `BTPROTO_RFCOMM`) to the BW150's MAC on channel 1
2. Sends `AT+BMDL24_BLE\r\n` — required by BW150 firmware to start data streaming
3. Parses 36-byte ATorch DC meter status packets (header `FF 55 01 02`)
4. Reports voltage, current, power, energy, temperature, and elapsed time

### Note on dl24.py

The upstream `dl24.py` (below) does **not** work with BW150 V2 hardware — it receives packets but fails to parse them (`receivedAns: False`). Use `bw150.py` instead.
